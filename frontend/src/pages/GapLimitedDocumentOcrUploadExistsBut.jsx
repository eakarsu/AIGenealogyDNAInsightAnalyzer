...
// Auto-generated page for: Limited document OCR (upload exists but no OCR pipeline visi

export default function GapLimitedDocumentOcrUploadExistsBut() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-limited-document-ocr-upload-exists-but', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '900px', margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>Limited document OCR (upload exists but no OCR pipeline visi</h1>
      <p style={{ color: '#666', marginBottom: '1.5rem' }}>Limited document OCR (upload exists but no OCR pipeline visible)</p>
      <form onSubmit={handleSubmit} style={{ marginBottom: '1.5rem' }}>
        ...
        >
          {loading ? 'Processing...' : 'Run Limited document OCR (upload exists but no OCR pipeline visi'}
        </button>
      </form>
